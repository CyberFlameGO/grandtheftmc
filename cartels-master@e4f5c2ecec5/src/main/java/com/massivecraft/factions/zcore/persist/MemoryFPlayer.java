...
public abstract class MemoryFPlayer implements FPlayer {
    ...
    //----------------------------------------------//
    // Title, Name, Faction CoreTag and Chat
    //----------------------------------------------//
    ...
    public boolean attemptClaim(Faction forFaction, Location location, boolean notifyFailure) {
        ...
        // if economy is enabled and they're not on the bypass list, make sure they can pay
        boolean mustPay = /*Econ.shouldBeUsed() &&*/Conf.econCostClaimWilderness > 0 && !this.isAdminBypassing() && !forFaction.isSafeZone() && !forFaction.isWarZone();
        double cost = 0.0;
...
//            }
            ...
            EcoResult result = getFaction().takeFromStash(cost);
            if(result == EcoResult.LOW_FUNDS) {
                msg("<h>%s<i> can't afford <h>%s<i> %s.", getFaction().describeTo(getFaction(), true), NumeralUtil.toCurrency(cost, Locale.US), TL.CLAIM_TOCLAIM.toString());
                return false;
            }

            if(result != EcoResult.SUCCESS) return false;
        }
