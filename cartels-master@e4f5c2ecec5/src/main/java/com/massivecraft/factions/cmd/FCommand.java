...
public abstract class FCommand extends MCommand<P> {
    ...
    public boolean isEnabled() {
        if (p.getLocked() && this.disableOnLock) {
            msg("<b>Cartels was locked by an admin. Please try again later.");
            return false;
        }

        if (this.isMoneyCommand && !Conf.econEnabled) {
            msg("<b>Cartels economy features are disabled on this server.");
            return false;
        }

        if (this.isMoneyCommand && !Conf.bankEnabled) {
            msg("<b>The cartel bank system is disabled on this server.");
            return false;
        }
    ...
    public boolean validSenderType(CommandSender sender, boolean informSenderIfNot) {
        ...
        if (!fme.hasFaction()) {
            sender.sendMessage(p.txt.parse("<b>You are not member of any cartel."));
            return false;
        }

        if (this.senderMustBeModerator && !fme.getRole().isAtLeast(Role.MODERATOR)) {
            sender.sendMessage(p.txt.parse("<b>Only cartel underbosses can %s.", this.getHelpShort()));
            return false;
        }

        if (this.senderMustBeAdmin && !fme.getRole().isAtLeast(Role.ADMIN)) {
            sender.sendMessage(p.txt.parse("<b>Only cartel bosses can %s.", this.getHelpShort()));
            return false;
        }
    ...
    public boolean assertHasFaction() {
        ...
        if (!fme.hasFaction()) {
            sendMessage("You are not member of any cartel.");
            return false;
        }
        return true;
    }
    ...
    public Faction strAsFaction(String name, Faction def, boolean msg) {
        ...
        if (msg && ret == null) {
            this.msg("<b>The cartel or player \"<p>%s<b>\" could not be found.", name);
        }
    ...
    public boolean canIAdministerYou(FPlayer i, FPlayer you) {
        if (!i.getFaction().equals(you.getFaction())) {
            i.sendMessage(p.txt.parse("%s <b>is not in the same cartel as you.", you.describeTo(i, true)));
            return false;
        }
        ...
        if (you.getRole().equals(Role.ADMIN)) {
            i.sendMessage(p.txt.parse("<b>Only the cartel admin can do that."));
        } else if (i.getRole().equals(Role.MODERATOR)) {
            if (i == you) {
                return true; //Moderators can control themselves
            } else {
                i.sendMessage(p.txt.parse("<b>Underbosses can't control each other..."));
            }
        } else {
            i.sendMessage(p.txt.parse("<b>You must be a cartel underboss to do that."));
        }
