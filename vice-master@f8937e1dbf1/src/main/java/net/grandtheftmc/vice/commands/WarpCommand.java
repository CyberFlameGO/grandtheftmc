    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            if (!s.hasPermission("warps.admin")) {
                Vice.getWorldManager().getWarpManager().warp(player, user, Vice.getUserManager().getLoadedUser(player.getUniqueId()),
                        new TaxiTarget(Vice.getWorldManager().getWarpManager().getRandomWarp()), 0, -1);
                return true;
            }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "load":
                Vice.getSettings().setWarpsConfig(Utils.loadConfig("warps"));
                Vice.getWorldManager().getWarpManager().loadWarps();
                s.sendMessage(Lang.WARP.f("&7Loaded Warps!"));
                return true;
            case "save":
                Vice.getWorldManager().getWarpManager().saveWarps();
