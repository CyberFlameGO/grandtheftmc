...
public class SpeedCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
        ...
        if(SpectatorCommand.getActiveStaff().contains(player.getName()) || user.isRank(UserRank.ADMIN)) {
            if(args.length != 1) {
                player.sendMessage(Lang.VICE.f("&7Usage: /speed <1-10>"));
            } else {
                ...
                player.sendMessage(Lang.VICE.f("&7Your " + (player.isFlying() ? "fly" : "walk") +
                        " speed has been set to &a" + Integer.min(Integer.valueOf(args[0]), 10)));
            }
        } else if(user.isRank(UserRank.HELPOP)) {
            player.sendMessage(Lang.VICE.f("&7You must be in spectator mode to use this command."));
        }
        return true;
    }
