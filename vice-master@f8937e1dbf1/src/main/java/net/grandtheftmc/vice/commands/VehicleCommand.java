    ...
    public boolean onCommand(CommandSender s, Command cmd, String lbl, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "list":
                ...
                List<VehicleProperties> vehicles = Vice.getWastedVehicles().getBabies().getVehicleProperties();
                if (args.length == 2) {
            ...
            case "give": {
                ...
                Player player = Bukkit.getPlayer(args[1]);
                Optional<VehicleProperties> opt = Vice.getWastedVehicles().getVehicle(args[2]);
...
//                    Core.sql.updateAsyncLater("update " + Core.name() + " set `" + vehicle.getIdentifier().toLowerCase() + "`=true where name='" + args[1] + "';");
                    ServerUtil.runTaskAsync(() -> ViceUserDAO.setVehiclePerm(args[1], vehicle, true));
...
//                    Core.sql.updateAsyncLater("update " + Core.name() + " set `" + vehicle.getIdentifier().toLowerCase() + "`=false where name='" + args[1] + "';");
                    ServerUtil.runTaskAsync(() -> ViceUserDAO.setVehiclePerm(args[1], vehicle, false));
