...
import net.grandtheftmc.vice.drugs.categories.examples.AAlcohol;
        ...
        DrugEffect effect = (drug, duration, player) -> {
            ...
            if (isPlayerDrunk(uuid)) {
                ...
                for (ItemStack is : player.getInventory().getStorageContents()) {
                    if (is != null && is.getType() != Material.CHEST && is.getType() != Material.WATCH) {
            ...
            Vice.getInstance().getDrugManager().getEffectManager().addEffect(player, (new PotionEffect(PotionEffectType.SLOW, effectDuration, 2 + (vodka ? 1 : 0))));
            Vice.getInstance().getDrugManager().getEffectManager().addEffect(player, (new PotionEffect(PotionEffectType.CONFUSION, effectDuration, 0)));
            Vice.getInstance().getDrugManager().getEffectManager().addEffect(player, (new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, effectDuration, 1 + (vodka ? 1 : 1))));
            Vice.getInstance().getDrugManager().getEffectManager().addEffect(player, (new PotionEffect(PotionEffectType.INCREASE_DAMAGE, effectDuration, 1 + (vodka ? 1 : 2))));
        };
