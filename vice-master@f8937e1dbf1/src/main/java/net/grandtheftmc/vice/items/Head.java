...
//        Core.sql.updateAsyncLater("insert into " + Core.name() + "_heads(sellerUUID, sellerName, head, expiry) values ('" + this.sellerUUID + "','" + this.sellerName + "','" + this.head + "'," + this.expiry + ");");
        ServerUtil.runTaskAsync(() -> ViceUserDAO.insertHead(sellerUUID, sellerName, head, expiry));
    }
    ...
    public ItemStack getItem() {
        return Utils.setSkullOwner(Utils.createItem(Material.SKULL_ITEM, 3, "&e&l" + this.head + "'s Head", "&7Value: &a$&l" + (this.hasBid() ? "10,000" : this.bid), "&7Sell me in the sewer!"), this.head);
    }
    ...
    public boolean paySeller() {
        ...
        if (seller == null)
            return false;
        ViceUser user = Vice.getUserManager().getLoadedUser(seller.getUniqueId());
        if (!this.hasBid()) {
...
//            Core.sql.updateAsyncLater("delete from " + Core.name() + "_heads where sellerUUID='" + this.sellerUUID + "' and head='" + this.head + "' and expiry=" + this.expiry + ';');
            ServerUtil.runTaskAsync(() -> ViceUserDAO.deleteHead(sellerUUID, head, expiry));
            Vice.getShopManager().removeHead(this);
            return true;
        }
        return false;
    }
...
//            Core.sql.updateAsyncLater("update " + Core.name() + " set bank=bank+" + this.bid + " where uuid='" + this.bidderUUID + "';");
            ServerUtil.runTaskAsync(() -> BaseDatabase.runCustomQuery("update " + Core.name() + " set bank=bank+" + this.bid + " where uuid='" + this.bidderUUID + "';"));
            this.bidderName = null;
            this.bidderUUID = null;
            this.bid = -1;
            return;
        }
        ViceUser user = Vice.getUserManager().getLoadedUser(bidder.getUniqueId());
        user.addMoney(this.bid);
        ViceUtils.updateBoard(bidder, user);
    ...
    public void bid(Player player, ViceUser user, double bid) {
        bid = Utils.round(bid);
