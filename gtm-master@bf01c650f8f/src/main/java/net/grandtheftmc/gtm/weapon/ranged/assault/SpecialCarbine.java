 ...
 */
public class SpecialCarbine extends AssultRifleWeapon implements RankedWeapon, WeaponVisualStatue {
    /**
    ...
    public SpecialCarbine() {
        super(
        		(short) 25,
                "Special Carbine", //Name
                WeaponType.ASSAULT, //Weapon Type
                AmmoType.ASSAULT_RIFLE, //AmmoType
                new ItemFactory(Material.DIAMOND_SWORD).setDurability((short) 241).build(), //ItemStack
        ...
        setOldItemStack(new ItemFactory(Material.STONE_HOE).setName(getName()).build());
        setDescription("Super solid gun choice;", "very versatile.");
        ...
        this.rps = 7;
        this.rpm = 445; //AssultRifleWeapon
    ...
    public Location spawnVisual(World world) {
        ...
        ArmorStand weapon = spawnEntity(origin.clone().add(-0.2, -0.18, 0.4), this, WeaponVisualStatue.VisualType.NONE);
        weapon.setArms(true);
...
//        hook.setMarker(true);
    ...
    public Location getOrigin(World world) {
        return new Location(world, -372.5, 25.5, 244.5, 90.0f, 0.0f);
    }
