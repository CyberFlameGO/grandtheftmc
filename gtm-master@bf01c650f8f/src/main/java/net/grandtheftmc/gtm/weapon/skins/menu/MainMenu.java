...
public class MainMenu extends CoreMenu {
    ...
    private void createMenuContent() {
        ...
        for (GameItem gameItem : GTM.getItemManager().getItems()) {
            if (gameItem.getType() == GameItem.ItemType.WEAPON) {
            	
                Optional<Weapon<?>> optional = GTM.getWastedGuns().getWeaponManager().getWeapon(gameItem.getWeaponOrVehicleOrDrug());

                if (optional.isPresent()) {
                    Weapon<?> weapon = optional.get();

                    if (weapon instanceof GoldMinigun || weapon instanceof Dildo)
                        continue;
        ...
        try {
            ...
            pageWeapons = weapons.subList(this.page * 15, weapons.size());
        }
        ...
        this.createNextPageButtons((int) Math.ceil((weapons.size()) / 15.0));
    ...
    private void createPreviousPageButtons() {
        for (int i = 0; i < 3; i++) {
            if (this.page != 0) {
                ItemStack stack = new ItemStack(Material.STAINED_GLASS_PANE, 1, DyeColor.RED.getWoolData());
                ItemMeta meta = stack.getItemMeta();
                meta.setDisplayName("Next Page");
                stack.setItemMeta(meta);
    ...
    private ItemStack createSkinButton(Weapon<?> weapon) {
        ...
        meta.setLore(Arrays.asList(
                ChatColor.translateAlternateColorCodes('&', "&6Unlocked:&r " + (1 + unlockedSkins) + "/" + weapon.getWeaponSkins().length), " ",
                ChatColor.translateAlternateColorCodes('&', "&7Click to view the skins for this weapon.")
        ));
